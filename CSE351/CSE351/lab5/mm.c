...
void* mm_malloc (size_t size) {
	...
	BlockInfo* nbp; // next block pointer
	BlockInfo* nfbp; // next free block pointer
	size_t bsr; // blockSize - reqSize
	...
	// look for a proper free list
	ptrFreeBlock = searchFreeList(reqSize);
	// create more space if no proper free list exist
	if(!ptrFreeBlock){
	...
	bsr=blockSize - reqSize;
	if(bsr >= MIN_BLOCK_SIZE){
		nbp = (BlockInfo*)UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize);
		nbp->sizeAndTags = bsr | TAG_PRECEDING_USED;
		nfbp = (BlockInfo*)UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize);
		// boundary tag of next block
		*((size_t*)UNSCALED_POINTER_SUB(nfbp, WORD_SIZE)) = nbp->sizeAndTags;
		insertFreeBlock(nbp);
	} else{
		nbp = (BlockInfo*)UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize);
		nbp->sizeAndTags = nbp->sizeAndTags | TAG_PRECEDING_USED;
	}
...
void mm_free (void *ptr){
	...
	// modify the info of current block and then info of next block
