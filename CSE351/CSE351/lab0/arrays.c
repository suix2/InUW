...
int main(int argc, char* argv[]) {
  ...
  // testing.
  // Answer: In all of the three cases, no errors or 
  //		warnings are reported by the compiler. However,
  //		a core dump fault may shows up when running the
  //		program, especially when the second argument is
  //		large, since the program is trying to modify
  //		some addresses out of the bound of the array.
  fillArray(array, 10);
  ...
  // Answer: In c, an array is a consecutive series of 
  //		addresses in memory represented by the address
  //		of the first member of the array. So fill in
  //		a number to an array with only one member in it
  //		is the same as assign a value to a variable by
  //		its address. Thus it it possible.
  fillArray(&value, 1);
  ...
  FourInts* test=(FourInts*)malloc(sizeof(FourInts));
