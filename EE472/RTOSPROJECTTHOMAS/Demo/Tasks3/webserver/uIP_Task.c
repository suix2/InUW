...
void ComsManage( void *pvParameters )
{
portBASE_TYPE i;
...
//		
	/* Find the start of the text to be displayed on the LCD. */
        ...
        *(myComs.cType)=*(pcText);
        if (*(pcText)=='T')
          sprintf(myComs.Payload,"%d", atoi(pcText+1));
        //*(myComs.Payload)=*(pcText+1);
        /* Write the message to the LCD. */
//	strcpy( cMessageForDisplay, pcText );
//	xMessage.pcMessage = ( signed portCHAR * ) cMessageForDisplay;
